# -*- coding: utf-8 -*-
"""
测试执行器
负责具体的测试执行逻辑，包括单次测试、连续测试、多频点测试等

从TestFlowController中提取的测试执行功能，遵循单一职责原则

Author: Jack
Date: 2025-01-30
"""

import logging
import math
import time
import threading
from typing import List, Dict, Any, Optional, Callable

from .channel_exception_manager import ChannelExceptionManager
from .battery_pre_test_checker import BatteryPreTestChecker
from backend.exceptions import ErrorCode, TestException
from utils.enhanced_exception_handler import handle_exceptions, handle_critical_exceptions

logger = logging.getLogger(__name__)


class TestExecutor:
    """
    测试执行器
    
    职责：
    - 单次测试执行
    - 连续测试执行
    - 多频点测试执行
    - 测试流程控制
    - 测试状态监控
    """
    
    def __init__(self, comm_manager, test_config_manager, device_config_manager,
                 impedance_data_manager, test_result_manager, startup_strategy_manager):
        """
        初始化测试执行器

        Args:
            comm_manager: 通信管理器
            test_config_manager: 测试配置管理器
            device_config_manager: 设备配置管理器
            impedance_data_manager: 阻抗数据管理器
            test_result_manager: 测试结果管理器
            startup_strategy_manager: 启动策略管理器
        """
        self.comm_manager = comm_manager
        self.test_config_manager = test_config_manager
        self.device_config_manager = device_config_manager
        self.impedance_data_manager = impedance_data_manager
        self.test_result_manager = test_result_manager
        self.startup_strategy_manager = startup_strategy_manager

        # Jack要求设置测试结果管理器的测试执行器引用，用于停止检查
        if self.test_result_manager:
            self.test_result_manager.test_executor = self

        # 测试控制
        self.stop_event = threading.Event()
        self.progress_callback = None
        self.status_callback = None

        # 连续测试计数
        self.continuous_test_count = 0

        # 连续测试统计数据
        self.continuous_test_statistics = {
            'start_time': None,
            'end_time': None,
            'cycle_times': [],  # 每轮测试时间
            'test_results': [],  # 所有测试结果
            'total_cycles': 0
        }

        # 添加当前活跃的测试管理器引用
        self._current_staggered_manager = None
        self._current_simultaneous_manager = None

        # 新增添加进度管理器用于连续进度计算
        from backend.test_progress_manager import TestProgressManager
        self.progress_manager = TestProgressManager()

        # 🚫 智能频点优化器已删除

        # 新增添加通道异常管理器
        self.exception_manager = ChannelExceptionManager(status_callback=self.status_callback)

        # 新增添加电池测试前检测器
        self.battery_checker = BatteryPreTestChecker(
            device_config_manager=self.device_config_manager,
            exception_manager=self.exception_manager,
            progress_callback=self.progress_callback
        )

        # 新增保存测试前电压值的字典（用于数据库存储）
        self.pre_test_voltages = {}  # {channel_num: voltage}

        # 新增电压保存锁定机制，防止测试过程中覆盖测试前电压
        self.voltage_locked_channels = set()  # 已锁定电压的通道集合

        # 新增设置测试结果管理器的测试执行器引用（用于获取测试前电压）
        if self.test_result_manager:
            self.test_result_manager.set_test_executor(self)
            logger.debug("已设置测试结果管理器的测试执行器引用")

        logger.info("测试执行器初始化完成")
    
    def set_progress_callback(self, callback: Callable):
        """设置进度回调函数"""
        self.original_progress_callback = callback  # 保存原始回调
        self.progress_callback = self._wrapped_progress_callback  # 使用包装回调

        # 同时设置阻抗数据管理器的回调，用于离群率计算
        if hasattr(self.impedance_data_manager, 'set_progress_callback'):
            self.impedance_data_manager.set_progress_callback(self.progress_callback)
        # 同时设置电池检测器的回调
        if hasattr(self, 'battery_checker'):
            self.battery_checker.progress_callback = self.progress_callback

    def _wrapped_progress_callback(self, channel_num: int, data: dict):
        """
        包装的进度回调函数，用于拦截电压更新并保存电压值

        Args:
            channel_num: 通道号
            data: 回调数据
        """
        try:
            # 修复电压保存锁定机制，只保存测试前的静置电压
            if data.get('state') == 'voltage_update' and 'voltage' in data:
                voltage = data['voltage']

                # 检查该通道的电压是否已被锁定
                if channel_num not in self.voltage_locked_channels:
                    # 首次保存电压，锁定该通道
                    self.pre_test_voltages[channel_num] = voltage
                    self.voltage_locked_channels.add(channel_num)
                    logger.info(f"✅ 保存并锁定通道{channel_num}测试前电压: {voltage:.3f}V")

                    # 新增UI显示也使用锁定的电压，更新data中的电压值
                    data['voltage'] = voltage
                    data['voltage_locked'] = True  # 标记电压已锁定
                else:
                    # 电压已锁定，使用锁定的电压值替换实时电压
                    locked_voltage = self.pre_test_voltages.get(channel_num, voltage)
                    data['voltage'] = locked_voltage
                    data['voltage_locked'] = True  # 标记电压已锁定
                    logger.debug(f"🔒 通道{channel_num}使用锁定电压: {locked_voltage:.3f}V，忽略实时电压: {voltage:.3f}V")

            # 调用原始回调函数
            if self.original_progress_callback:
                self.original_progress_callback(channel_num, data)

        except Exception as e:
            logger.error(f"包装进度回调处理失败: {e}")
            # 即使包装回调失败，也要调用原始回调
            if self.original_progress_callback:
                self.original_progress_callback(channel_num, data)

    def set_status_callback(self, callback: Callable):
        """设置状态回调函数"""
        self.status_callback = callback
        # 同时更新异常管理器的状态回调
        if hasattr(self, 'exception_manager'):
            self.exception_manager.status_callback = callback

    def set_stop_event(self, stop_event: threading.Event):
        """设置停止事件"""
        self.stop_event = stop_event
    
    @handle_critical_exceptions()
    def execute_test(self, test_config: Dict[str, Any], enabled_channels: List[int]) -> bool:
        """
        执行测试
        
        Args:
            test_config: 测试配置
            enabled_channels: 启用的通道列表
            
        Returns:
            是否执行成功
        """
        try:
            logger.info("开始执行测试")

            # 🔍 [DEBUG] 重置stop_event，确保新测试开始时处于清除状态
            if self.stop_event.is_set():
                logger.warning(f"🔍 [DEBUG] 检测到stop_event已设置，清除后开始新测试")
                self.stop_event.clear()
            logger.info(f"🔍 [DEBUG] 测试开始时stop_event状态: {self.stop_event.is_set()}")

            # 🔍 [SINGLE_FREQ_DEBUG] 检查是否为单频点测试
            frequencies = test_config.get('frequencies', [])
            if len(frequencies) == 1:
                logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 检测到单频点测试: {frequencies[0]}Hz")
                logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 测试配置: {test_config}")

            # 新增重置通道异常状态，开始新测试
            self.exception_manager.reset_for_new_test()

            # � 智能频点优化已删除
            frequencies = test_config.get('frequencies', [])
            test_config['optimization_used'] = False
            logger.info(f"📊 使用全频段测试: {len(frequencies)}个频点")

            # 新增启动测试计时，估算总测试时间
            estimated_duration = len(test_config['frequencies']) * 30.0  # 估算每个频点30秒
            self.progress_manager.start_test_timer(estimated_duration)

            # 新增清空之前保存的电压值和锁定状态
            self.pre_test_voltages.clear()
            self.voltage_locked_channels.clear()
            logger.info("✅ 已清空之前保存的测试前电压值和锁定状态")

            # 记录测试开始时间
            self.test_result_manager.record_test_start(enabled_channels)

            # 准备测试环境
            if not self._prepare_test_environment(test_config):
                logger.error("测试环境准备失败")
                return False
            
            # 检查测试配置
            continuous_mode = test_config.get('continuous_mode', False)
            logger.debug(f"测试配置检查 - continuous_mode: {continuous_mode}")
            logger.debug(f"完整测试配置: {test_config}")

            # 根据测试模式执行不同的测试流程
            if continuous_mode:
                logger.info("开始连续测试模式")
                success = self._execute_continuous_test(test_config, enabled_channels)
            else:
                logger.info("开始单次测试模式")
                success = self._execute_single_test(test_config, enabled_channels)
            
            # 记录测试结束时间
            self.test_result_manager.record_test_end(enabled_channels)

            # 关键修复测试完成后调用状态回调，通知测试流程控制器重置状态
            if self.status_callback:
                try:
                    logger.info(f"🔄 测试执行完成，发送状态回调通知测试流程控制器重置状态")
                    self.status_callback({'is_testing': False, 'action': 'test_completed', 'success': success})
                    logger.info("✅ 测试完成状态回调已发送")
                except Exception as e:
                    logger.error(f"❌ 发送测试完成状态回调失败: {e}")
            else:
                logger.warning("⚠️ status_callback 为空，无法发送测试完成通知")

            # 🔧 新增：强制发送测试完成信号，确保UI状态重置
            try:
                logger.info("🔄 强制发送测试完成信号，确保UI状态重置...")

                # 通过进度回调发送全局测试完成信号
                if self.progress_callback:
                    self.progress_callback(0, {  # 通道0表示全局信号
                        'state': 'test_completed',
                        'progress': 100,
                        'message': '测试完成',
                        'action': 'force_ui_reset',
                        'success': success,
                        'timestamp': time.time()
                    })
                    logger.info("✅ 全局测试完成信号已发送")

            except Exception as e:
                logger.error(f"❌ 发送全局测试完成信号失败: {e}")

            logger.info(f"测试执行完成: {'成功' if success else '失败'}")
            return success
            
        except Exception as e:
            logger.error(f"测试执行失败: {e}")
            import traceback
            logger.error(f"异常详情: {traceback.format_exc()}")
            return False
    
    def _prepare_test_environment(self, test_config: Dict[str, Any]) -> bool:
        """
        准备测试环境

        Args:
            test_config: 测试配置

        Returns:
            是否准备成功
        """
        try:
            logger.info("开始准备测试环境...")

            # 检查测试配置
            logger.debug(f"测试配置检查: {test_config}")
            enabled_channels = test_config.get('enabled_channels', list(range(1, 9)))
            logger.debug(f"启用通道: {enabled_channels}")

            if not enabled_channels:
                logger.error("❌ 启用通道为空，无法进行测试")
                return False

            # 1. 检查设备连接
            logger.debug("检查设备连接...")
            try:
                if not self.device_config_manager.is_device_ready():
                    logger.error("设备未准备就绪")
                    # 尝试重新连接设备
                    logger.debug("尝试重新检查设备连接...")
                    if hasattr(self.device_config_manager, 'check_device_connection'):
                        connection_ok = self.device_config_manager.check_device_connection()
                        logger.debug(f"设备连接重新检查结果: {connection_ok}")
                        if not connection_ok:
                            return False
                    else:
                        return False
                else:
                    logger.debug("设备连接正常")
            except Exception as e:
                logger.error(f"❌ 设备连接检查失败: {e}")
                return False

            # 2. 设备参数配置已在测试控制组件中完成，这里跳过
            logger.debug("设备参数配置已在开始测试时完成，跳过重复配置")

            # 3. 更新电压显示
            logger.debug("更新电压显示...")
            try:
                self.device_config_manager.update_voltage_display(enabled_channels, self.progress_callback)
                logger.debug("电压显示更新完成")
            except Exception as e:
                logger.error(f"电压显示更新失败: {e}")
                # 电压显示失败不应该阻止测试继续
                logger.warning("电压显示更新失败，但测试将继续进行")

            # 4. 等待设备准备
            logger.debug("等待设备准备...")
            time.sleep(0.5)

            logger.info("测试环境准备完成")
            return True

        except Exception as e:
            logger.error(f"❌ 准备测试环境失败: {e}")
            import traceback
            logger.error(f"❌ 详细错误信息: {traceback.format_exc()}")
            return False
    
    @handle_exceptions(show_dialog=True, auto_retry=False, critical=True)
    def _execute_single_test(self, test_config: Dict[str, Any], enabled_channels: List[int]) -> bool:
        """
        执行单次测试
        
        Args:
            test_config: 测试配置
            enabled_channels: 启用的通道列表
            
        Returns:
            是否执行成功
        """
        try:
            # 获取测试配置信息
            frequencies = test_config.get('frequencies', [1.0])
            is_single_mode = test_config.get('is_single_mode', False)
            frequency_order = test_config.get('frequency_order', 'high_to_low')

            # 新增设置测试频点到阻抗数据管理器（用于正确的序号生成）
            if hasattr(self.impedance_data_manager, 'set_test_frequencies'):
                self.impedance_data_manager.set_test_frequencies(frequencies)
                logger.debug(f"已设置测试频点到阻抗数据管理器: {len(frequencies)}个频点")

            # 输出测试模式信息
            order_desc = "高频到低频" if frequency_order == 'high_to_low' else "低频到高频"
            logger.info(f"测试模式: 多频点测试 ({order_desc})")
            logger.info(f"测试频率: {frequencies}")
            return self._execute_multi_frequency_test(frequencies, enabled_channels, test_config)
                
        except Exception as e:
            logger.error(f"单次测试执行失败: {e}")
            return False

    def _pre_test_battery_check(self, enabled_channels: List[int]) -> List[int]:
        """
        测试前电池检测和通道预筛选

        Args:
            enabled_channels: 启用的通道列表

        Returns:
            有电池连接的有效通道列表
        """
        try:
            logger.info(f"🔋 开始测试前电池检测，检查通道: {enabled_channels}")

            # 使用电池检测器进行检测
            return self.battery_checker.get_valid_channels_only(enabled_channels)

        except Exception as e:
            logger.error(f"测试前电池检测失败: {e}")
            # 出错时返回原始通道列表，避免阻塞测试
            return enabled_channels

    def _mark_channel_as_no_battery(self, channel_num: int, reason: str):
        """
        标记通道为无电池状态

        Args:
            channel_num: 通道号
            reason: 无电池的原因
        """
        try:
            # 通过异常管理器标记通道异常
            from backend.channel_exception_manager import ChannelExceptionType, ChannelExceptionInfo
            from datetime import datetime

            exception_info = ChannelExceptionInfo(
                channel_number=channel_num,
                exception_type=ChannelExceptionType.BATTERY_ERROR,
                status_code=0x0003,  # 使用电池错误状态码
                error_message=f"无电池: {reason}",
                detection_time=datetime.now(),
                frequency_when_detected=None,
                should_skip=True
            )

            # 添加到异常管理器
            self.exception_manager.exception_channels[channel_num] = exception_info
            self.exception_manager.skipped_channels.add(channel_num)

            # 通知UI更新状态
            if self.progress_callback:
                self.progress_callback(channel_num, {
                    'state': 'no_battery',
                    'progress': 0,
                    'message': f'无电池: {reason}',
                    'exception_type': 'BATTERY_ERROR',
                    'error_message': f'无电池: {reason}',
                    'voltage': 0.0
                })

            logger.info(f"🚫 通道{channel_num}已标记为无电池: {reason}")

        except Exception as e:
            logger.error(f"标记通道{channel_num}无电池状态失败: {e}")


    
    @handle_exceptions(show_dialog=True, auto_retry=True, max_retries=2, critical=True)
    def _execute_multi_frequency_test(self, frequencies: List[float], enabled_channels: List[int],
                                    test_config: Dict[str, Any]) -> bool:
        """
        执行多频点测试（支持并行错频模式）

        Args:
            frequencies: 频率列表
            enabled_channels: 启用的通道列表
            test_config: 测试配置

        Returns:
            是否执行成功
        """
        try:
            logger.info(f"开始多频点测试，共{len(frequencies)}个频点")

            # 新增测试前电池检测和通道预筛选
            valid_channels = self._pre_test_battery_check(enabled_channels)
            if not valid_channels:
                logger.warning("所有通道都无电池连接，跳过测试")
                return False

            if len(valid_channels) < len(enabled_channels):
                logger.info(f"电池检测完成，有效通道: {valid_channels}，跳过通道: {[ch for ch in enabled_channels if ch not in valid_channels]}")

            enabled_channel_indices = [ch - 1 for ch in valid_channels]  # 转换为0基索引

            # 检查是否启用并行错频模式
            use_staggered_mode = test_config.get('use_parallel_staggered_mode', True)
            critical_frequency = test_config.get('critical_frequency', 10.0)

            print(f"🔍 [测试执行器] 并行错频模式检查: use_staggered_mode={use_staggered_mode}, critical_frequency={critical_frequency}")

            if use_staggered_mode:
                print(f"🎯 [测试执行器] 选择并行错频测试路径")
                logger.info(f"🔄 启用并行错频模式，临界频率: {critical_frequency}Hz")
                return self._execute_parallel_staggered_test(frequencies, enabled_channel_indices, test_config)
            else:
                print(f"🔄 [测试执行器] 选择传统同时启动测试路径")
                logger.info("🚀 使用传统同时启动模式")
                return self._execute_traditional_test(frequencies, enabled_channel_indices, test_config)

        except Exception as e:
            logger.error(f"多频点测试失败: {e}")
            return False

    def _execute_parallel_staggered_test(self, frequencies: List[float], enabled_channel_indices: List[int],
                                       test_config: Dict[str, Any]) -> bool:
        """
        执行并行错频测试

        Args:
            frequencies: 频率列表
            enabled_channel_indices: 启用的通道索引列表（0-7）
            test_config: 测试配置

        Returns:
            是否执行成功
        """
        try:
            from backend.parallel_staggered_test_manager import ParallelStaggeredTestManagerSimplified as ParallelStaggeredTestManager, ParallelStaggeredTestConfig, ParallelStaggeredTestState

            logger.info("开始并行错频测试")

            # 创建并行错频测试配置
            staggered_config = ParallelStaggeredTestConfig()
            staggered_config.enabled_channels = enabled_channel_indices
            staggered_config.frequencies = frequencies
            staggered_config.critical_frequency = test_config.get('critical_frequency', 10.0)
            staggered_config.timeout_seconds = test_config.get('timeout_seconds', 120)
            staggered_config.status_check_interval = test_config.get('status_check_interval', 0.2)

            # 创建并行错频测试管理器
            # 修复传递停止事件和阻抗数据管理器给并行错频测试管理器
            staggered_manager = ParallelStaggeredTestManager(
                self.comm_manager,
                impedance_data_manager=self.impedance_data_manager
            )

            # 修复确保停止事件正确传递给并行错频测试管理器
            if hasattr(staggered_manager, 'stop_event'):
                staggered_manager.stop_event = self.stop_event
                logger.info("✅ 停止事件已传递给并行错频测试管理器")

            # 修复保存管理器引用以便停止时使用
            self._current_staggered_manager = staggered_manager

            # 设置进度回调
            def staggered_progress_callback(progress_info):
                # 整体进度回调
                pass

            def channel_progress_callback(channel_num, progress_data):
                if self.progress_callback:
                    # 修复使用锁定的测试前电压，而不是实时电压
                    locked_voltage = self.pre_test_voltages.get(channel_num)
                    if locked_voltage is not None:
                        # 使用锁定的测试前电压
                        progress_data['voltage'] = locked_voltage
                        progress_data['voltage_locked'] = True
                        logger.debug(f"通道{channel_num}并行错频测试使用锁定电压: {locked_voltage:.3f}V")
                    else:
                        # 如果没有锁定电压，尝试读取实时电压作为备用
                        try:
                            voltage = self.device_config_manager.read_channel_voltage(channel_num)
                            if voltage is not None and voltage > 0:
                                progress_data['voltage'] = voltage
                                progress_data['voltage_locked'] = False
                                logger.debug(f"通道{channel_num}并行错频测试使用实时电压: {voltage:.3f}V")
                        except Exception as e:
                            logger.debug(f"获取通道{channel_num}电压失败: {e}")

                    self.progress_callback(channel_num, progress_data)

            # 设置回调函数
            staggered_manager.set_progress_callback(staggered_progress_callback)
            staggered_manager.set_channel_progress_callback(channel_progress_callback)

            logger.debug("并行错频测试管理器回调函数已设置")

            # 启动并行错频测试
            success = staggered_manager.start_test(staggered_config)
            print(f"🚀 [测试执行器] start_parallel_staggered_test返回: {success}")

            if success:
                # 等待测试完成
                while staggered_manager.get_test_state() != ParallelStaggeredTestState.COMPLETED and not self.stop_event.is_set():
                    if staggered_manager.get_test_state() == ParallelStaggeredTestState.ERROR:
                        logger.error("并行错频测试出现错误")
                        break
                    time.sleep(0.1)

                # 检查停止标志
                if self.stop_event.is_set():
                    logger.warning("⚠️ 并行错频测试检测到停止标志，不保存数据")
                    return False

                # 检查测试是否因错误而停止
                if staggered_manager.get_test_state() == ParallelStaggeredTestState.ERROR:
                    logger.error("❌ 并行错频测试出现错误，不保存数据")
                    return False

                # 获取测试结果
                results = staggered_manager.get_test_results()

                # 🔧 [重复保存修复] 移除重复的数据保存逻辑
                # 数据保存已由数据收集器(test_data_collector.py)统一处理，避免重复保存
                if results:
                    logger.info(f"并行错频测试完成，获取到{len(results)}个频点的数据")
                    logger.debug("数据保存由数据收集器统一处理，避免重复保存明细数据")

                # Jack要求只有在测试自然完成（未被停止）时才进行EIS分析和结果显示
                if not self.stop_event.is_set():
                    logger.info("✅ 并行错频测试自然完成，开始EIS分析和结果显示")
                    # 处理测试完成
                    self._process_test_completion(enabled_channel_indices, test_config)
                else:
                    logger.info("🛑 并行错频测试检测到停止标志，跳过EIS分析和结果显示，避免计算脏数据")

                logger.info("✅ 并行错频测试完成")
                return True
            else:
                logger.error("❌ 并行错频测试启动失败")
                return False

        except Exception as e:
            logger.error(f"并行错频测试失败: {e}")
            import traceback
            logger.error(f"异常详情: {traceback.format_exc()}")
            return False

    def _execute_traditional_test(self, frequencies: List[float], enabled_channel_indices: List[int],
                                test_config: Dict[str, Any]) -> bool:
        """
        执行传统同时启动测试

        Args:
            frequencies: 频率列表
            enabled_channel_indices: 启用的通道索引列表（0-7）
            test_config: 测试配置

        Returns:
            是否执行成功
        """
        try:
            logger.info("🚀 开始传统同时启动测试")

            # 🔍 [SINGLE_FREQ_DEBUG] 传统测试开始调试
            if len(frequencies) == 1:
                logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 传统测试开始，单频点: {frequencies[0]}Hz")
                logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 启用通道: {[ch + 1 for ch in enabled_channel_indices]}")

            enabled_channels = [ch_idx + 1 for ch_idx in enabled_channel_indices]

            # 保存当前测试的频点索引，用于正确显示进度
            self.current_frequency_index = 0

            for i, frequency in enumerate(frequencies, 1):
                if self.stop_event.is_set():
                    logger.info("检测到停止标志，停止当前频点测量")
                    break

                logger.info(f"测试频点 {i}/{len(frequencies)}: {frequency}Hz")

                # 更新当前频点索引
                self.current_frequency_index = i

                # 1. 设置频率
                if not self.comm_manager.set_frequency_broadcast(frequency):
                    logger.error(f"设置频率{frequency}Hz失败")
                    continue

                # 优化通知频点更新（为所有启用的通道）- 使用线性进度映射
                if self.progress_callback:
                    # 使用线性进度映射每完成一个频点，进度增加 (90% / 总频点数)
                    completed_frequencies = i - 1  # 已完成的频点数（i是当前正在测试的频点索引，从1开始）
                    progress_per_frequency = 90.0 / len(frequencies)  # 每个频点占用的进度范围
                    base_progress = 5 + (completed_frequencies * progress_per_frequency)  # 基础进度：5% + 已完成频点的进度

                    for channel_num in enabled_channels:
                        # 检查通道是否异常
                        if self.exception_manager.is_channel_skipped(channel_num):
                            # 异常通道显示异常状态
                            exception_info = self.exception_manager.get_exception_info(channel_num)
                            if exception_info:
                                self.progress_callback(channel_num, {
                                    'state': 'exception',
                                    'progress': 0,
                                    'message': f'通道异常: {exception_info.error_message}',
                                    'exception_type': exception_info.exception_type.value,
                                    'status_code': exception_info.status_code,
                                    'error_message': exception_info.error_message
                                })
                            continue

                        # 正常通道显示测试进度
                        # 读取当前通道电压
                        voltage = self.device_config_manager.read_channel_voltage(channel_num)
                        if voltage is None:
                            voltage = 0.0

                        # 纯频点进度：已完成频点占百分比
                        progress_value = int(base_progress)

                        self.progress_callback(channel_num, {
                            'state': 'testing',
                            'progress': progress_value,
                            'message': f'频点测试: {frequency}Hz',
                            'frequency': frequency,
                            'frequency_index': i,  # 从1开始的频点索引
                            'total_frequencies': len(frequencies),
                            'voltage': voltage,  # 添加电压数据
                            'completed_frequencies': completed_frequencies,  # 已完成频点数
                            'progress_calculation': 'time_based',  # 标识使用时间进度
                            'time_based_progress': True
                        })

                        logger.debug(f"通道{channel_num}进度更新: {int(base_progress)}%")

                # 新增过滤出正常通道进行测试
                normal_channels = self.exception_manager.get_normal_channels(enabled_channels)
                normal_channel_indices = [ch - 1 for ch in normal_channels]  # 转换为0基索引

                if not normal_channels:
                    logger.warning(f"频点{frequency}Hz所有通道都异常，跳过该频点")
                    continue

                logger.info(f"频点{frequency}Hz正常通道: {normal_channels}")

                # 2. 启动阻抗测量（只对正常通道）
                success = self.comm_manager.start_impedance_measurement_broadcast(normal_channel_indices)

                if not success:
                    logger.error(f"启动阻抗测量失败: {frequency}Hz")
                    continue

                # 3. 等待测量完成（只等待正常通道）
                measurement_completed = self._wait_for_measurement_completion(normal_channel_indices, frequency)
                if not measurement_completed:
                    if self.stop_event.is_set():
                        logger.info(f"频点{frequency}Hz检测到停止标志，尝试读取已完成的数据")
                    else:
                        logger.warning(f"频点{frequency}Hz测量超时")

                # 4. 读取阻抗数据（只读取正常通道的数据）
                logger.debug(f"开始读取频点{frequency}Hz的阻抗数据（正常通道: {normal_channels}）...")
                impedance_data = self.impedance_data_manager.read_impedance_data_for_frequency(
                    frequency, normal_channel_indices
                )

                # 5. 保存测试数据 - 只有在测试未停止时才保存
                if impedance_data and not self.stop_event.is_set():
                    batch_id = self.test_result_manager.current_batch_id
                    if batch_id:
                        self.impedance_data_manager.save_impedance_data(frequency, impedance_data, batch_id)
                        logger.info(f"频点{frequency}Hz阻抗数据已保存，通道数: {len(impedance_data.get('channels', {}))}")
                    else:
                        logger.warning(f"频点{frequency}Hz阻抗数据未保存：批次ID为空")
                elif impedance_data and self.stop_event.is_set():
                    logger.warning(f"⚠️ 测试已被停止，跳过频点{frequency}Hz阻抗数据保存")
                elif not impedance_data:
                    logger.warning(f"频点{frequency}Hz阻抗数据读取失败或为空")

                # 6. 如果测量被停止，则退出循环（不再保存任何数据）
                if self.stop_event.is_set():
                    logger.warning(f"⚠️ 测试被停止，退出测试循环，不保存后续数据")
                    break

                # 7. 如果测量超时且无数据，继续下一个频点
                if not measurement_completed and not impedance_data:
                    continue

                # 新增频点完成后的进度更新 - 平滑递进到下一个频点
                if self.progress_callback:
                    # 计算频点完成后的进度：当前频点完成，进度应该到达下一个频点的起始位置
                    completed_frequencies = i  # 当前频点已完成（i是当前频点索引，从1开始）
                    progress_per_frequency = 90.0 / len(frequencies)  # 每个频点占用的进度范围
                    completed_progress = 5 + (completed_frequencies * progress_per_frequency)  # 完成进度：5% + 已完成频点的进度

                    for channel_num in enabled_channels:
                        # 读取当前通道电压
                        voltage = self.device_config_manager.read_channel_voltage(channel_num)
                        if voltage is None:
                            voltage = 0.0

                        # 发送频点完成进度更新
                        self.progress_callback(channel_num, {
                            'state': 'frequency_completed',
                            'progress': int(completed_progress),
                            'message': f'频点{frequency}Hz完成',
                            'frequency': frequency,
                            'frequency_index': i,  # 当前完成的频点索引
                            'total_frequencies': len(frequencies),
                            'voltage': voltage,
                            'completed_frequencies': completed_frequencies,  # 已完成频点数
                            'progress_calculation': 'frequency_completed'  # 标识为频点完成进度
                        })

                        logger.debug(f"通道{channel_num}频点完成: {frequency}Hz, 进度={int(completed_progress)}%")

            logger.info("传统同时启动测试完成")

            # 🔧 新增：验证频点采集完整性
            logger.info(f"🔍 [DEBUG] 开始验证频点完整性，预期频点: {frequencies}")
            if len(frequencies) == 1:
                logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 单频点验证开始: {frequencies[0]}Hz")

            validation_result = self._validate_multi_frequency_test_completeness(frequencies, [ch + 1 for ch in enabled_channel_indices])
            logger.info(f"🔍 [DEBUG] 频点验证结果: overall_success={validation_result.get('overall_success', True)}")
            logger.info(f"🔍 [DEBUG] 验证详情: {validation_result}")

            if len(frequencies) == 1:
                logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 单频点验证结果: {validation_result}")

            # 如果验证失败，标记测试结果为数据错误
            if not validation_result.get('overall_success', True):
                logger.error("❌ 频点数据验证失败，将标记测试结果为数据错误状态")
                logger.error(f"🔍 [DEBUG] 验证失败原因: {validation_result.get('summary', '未知')}")
                if len(frequencies) == 1:
                    logger.error(f"🔍 [SINGLE_FREQ_DEBUG] 单频点验证失败！这可能是问题所在")
                self._mark_test_as_data_error(enabled_channel_indices, validation_result)
                # 🔍 [DEBUG] 注意：即使验证失败，也不应该设置stop_event
                logger.info(f"🔍 [DEBUG] 数据错误标记完成，stop_event状态: {self.stop_event.is_set()}")
            else:
                logger.info("✅ 频点数据验证通过")
                if len(frequencies) == 1:
                    logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 单频点验证通过")

            # Jack要求只有在测试自然完成（未被停止）时才进行EIS分析和结果显示
            # 🔍 [DEBUG] 添加详细的stop_event状态日志
            logger.info(f"🔍 [DEBUG] 测试循环结束，stop_event状态: {self.stop_event.is_set()}")
            logger.info(f"🔍 [DEBUG] 频点数量: {len(frequencies)}, 启用通道: {[ch + 1 for ch in enabled_channel_indices]}")

            if len(frequencies) == 1:
                logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 单频点测试循环结束，stop_event状态: {self.stop_event.is_set()}")

            if not self.stop_event.is_set():
                logger.info("✅ 测试自然完成，开始EIS分析和结果显示")
                logger.info("🔍 [DEBUG] 准备调用_process_test_completion")
                if len(frequencies) == 1:
                    logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 单频点测试准备调用_process_test_completion")
                # 6. 测试完成后进行EIS分析和结果显示
                self._process_test_completion(enabled_channel_indices, test_config)
                logger.info("🔍 [DEBUG] _process_test_completion调用完成")
                if len(frequencies) == 1:
                    logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 单频点_process_test_completion调用完成")
            else:
                logger.error("🛑 检测到停止标志，跳过EIS分析和结果显示，避免计算脏数据")
                logger.error(f"🔍 [DEBUG] stop_event被设置，无法保存测试数据！")
                if len(frequencies) == 1:
                    logger.error(f"🔍 [SINGLE_FREQ_DEBUG] 单频点测试被停止！这是数据无法保存的原因！")

            return True

        except Exception as e:
            logger.error(f"传统同时启动测试失败: {e}")
            return False

    def _validate_multi_frequency_test_completeness(self, expected_frequencies: List[float], enabled_channels: List[int]) -> Dict[str, Any]:
        """
        验证多频点测试的完整性

        Args:
            expected_frequencies: 预期的频率列表
            enabled_channels: 启用的通道列表

        Returns:
            验证结果字典
        """
        try:
            logger.info(f"🔍 开始验证频点采集完整性...")
            logger.info(f"   预期频点数量: {len(expected_frequencies)}个")
            logger.info(f"   测试通道: {enabled_channels}")

            # 初始化验证结果
            validation_result = {
                'overall_success': True,
                'expected_count': len(expected_frequencies),
                'channel_results': {},
                'critical_failures': [],
                'summary': ''
            }

            # 检查每个通道的频点数据完整性
            overall_issues = []

            for channel_num in enabled_channels:
                try:
                    # 从阻抗数据管理器获取该通道的数据
                    channel_data = self.impedance_data_manager.get_channel_impedance_data(channel_num)

                    if not channel_data:
                        overall_issues.append(f"通道{channel_num}: 无任何阻抗数据")
                        logger.error(f"❌ 通道{channel_num}: 无任何阻抗数据")
                        continue

                    # 检查实际采集的频点
                    actual_frequencies = list(channel_data.keys())
                    actual_count = len(actual_frequencies)
                    expected_count = len(expected_frequencies)

                    # 找出缺失的频点
                    missing_frequencies = [freq for freq in expected_frequencies if freq not in actual_frequencies]
                    missing_count = len(missing_frequencies)

                    # 保存通道验证结果
                    channel_result = {
                        'actual_count': actual_count,
                        'missing_count': missing_count,
                        'missing_frequencies': missing_frequencies,
                        'missing_ratio': missing_count / expected_count if expected_count > 0 else 0,
                        'status': 'complete' if missing_count == 0 else 'incomplete'
                    }
                    validation_result['channel_results'][channel_num] = channel_result

                    if missing_count == 0:
                        logger.info(f"✅ 通道{channel_num}: 频点数据完整 ({actual_count}/{expected_count}个频点)")
                    else:
                        missing_ratio = missing_count / expected_count

                        # 生成缺失频点的简要描述
                        missing_freq_str = ", ".join([f"{freq}Hz" for freq in missing_frequencies[:3]])
                        if missing_count > 3:
                            missing_freq_str += f" 等{missing_count}个"

                        if missing_ratio < 0.1:  # 缺失少于10%
                            logger.warning(f"⚠️ 通道{channel_num}: 轻微缺失 ({actual_count}/{expected_count}个频点)")
                            logger.warning(f"   缺失频点: {missing_freq_str}")
                            overall_issues.append(f"通道{channel_num}: 轻微缺失{missing_count}个频点")
                            channel_result['severity'] = 'minor'
                        elif missing_ratio < 0.3:  # 缺失10%-30%
                            logger.error(f"❌ 通道{channel_num}: 严重缺失 ({actual_count}/{expected_count}个频点)")
                            logger.error(f"   缺失频点: {missing_freq_str}")
                            overall_issues.append(f"通道{channel_num}: 严重缺失{missing_count}个频点")
                            channel_result['severity'] = 'major'
                        else:  # 缺失超过30%
                            logger.error(f"🚨 通道{channel_num}: 数据严重不完整 ({actual_count}/{expected_count}个频点)")
                            logger.error(f"   缺失频点: {missing_freq_str}")
                            overall_issues.append(f"通道{channel_num}: 数据严重不完整，缺失{missing_count}个频点")
                            channel_result['severity'] = 'critical'
                            validation_result['critical_failures'].append(channel_num)
                            validation_result['overall_success'] = False

                except Exception as e:
                    logger.error(f"❌ 验证通道{channel_num}频点数据失败: {e}")
                    overall_issues.append(f"通道{channel_num}: 验证失败")
                    validation_result['critical_failures'].append(channel_num)
                    validation_result['overall_success'] = False

            # 生成总体验证报告
            if not overall_issues:
                logger.info(f"🎉 频点采集验证通过: 所有通道数据完整")
                validation_result['summary'] = "所有通道频点数据完整"
            else:
                logger.warning(f"⚠️ 频点采集验证发现问题:")
                for issue in overall_issues:
                    logger.warning(f"   - {issue}")

                if validation_result['critical_failures']:
                    logger.error(f"🚨 严重问题: 通道{validation_result['critical_failures']}数据严重不足，无法进行可靠分析")
                    validation_result['summary'] = f"通道{validation_result['critical_failures']}数据严重不足"
                else:
                    logger.warning(f"💡 建议: 如果缺失较多，请检查设备连接或重新测试")
                    validation_result['summary'] = f"发现{len(overall_issues)}个通道有数据缺失"

            return validation_result

        except Exception as e:
            logger.error(f"验证多频点测试完整性失败: {e}")
            return {
                'overall_success': False,
                'expected_count': len(expected_frequencies),
                'channel_results': {},
                'critical_failures': enabled_channels,  # 验证失败，标记所有通道为失败
                'summary': f'验证失败: {str(e)}'
            }

    def _mark_test_as_data_error(self, enabled_channel_indices: List[int], validation_result: Dict[str, Any]):
        """
        将测试结果标记为数据错误状态

        Args:
            enabled_channel_indices: 启用的通道索引列表（0-7）
            validation_result: 验证结果
        """
        try:
            logger.info("🔧 开始标记数据错误的测试结果...")

            # 获取当前批次ID
            batch_id = self.test_result_manager.current_batch_id
            if not batch_id:
                logger.error("无法获取当前批次ID，无法标记数据错误状态")
                return

            # 为每个有严重问题的通道标记数据错误
            critical_channels = validation_result.get('critical_failures', [])

            for channel_num in critical_channels:
                try:
                    # 获取通道的验证结果
                    channel_result = validation_result['channel_results'].get(channel_num, {})
                    missing_count = channel_result.get('missing_count', 0)
                    expected_count = validation_result.get('expected_count', 0)
                    missing_ratio = channel_result.get('missing_ratio', 0)

                    # 构造错误信息
                    error_message = f"频点数据严重不足：预期{expected_count}个频点，实际采集{expected_count - missing_count}个，缺失{missing_ratio:.1%}，无法进行可靠的EIS分析"

                    # 保存数据错误的测试结果
                    error_result = {
                        'channel': channel_num,
                        'batch_id': batch_id,
                        'test_status': 'DATA_ERROR',  # 新增状态：数据错误
                        'rs': 0.0,
                        'rct': 0.0,
                        'w_impedance': 0.0,
                        'error_message': error_message,
                        'validation_info': {
                            'expected_frequencies': expected_count,
                            'actual_frequencies': expected_count - missing_count,
                            'missing_frequencies': missing_count,
                            'missing_ratio': missing_ratio
                        }
                    }

                    # 保存到数据库
                    self.test_result_manager.save_test_result(error_result)

                    logger.error(f"❌ 通道{channel_num}已标记为数据错误状态: {error_message}")

                except Exception as e:
                    logger.error(f"标记通道{channel_num}数据错误状态失败: {e}")

            logger.info(f"✅ 数据错误标记完成，影响通道: {critical_channels}")

        except Exception as e:
            logger.error(f"标记测试数据错误状态失败: {e}")

    @handle_exceptions(show_dialog=True, auto_retry=True, max_retries=3, critical=True)
    def _execute_continuous_test(self, test_config: Dict[str, Any], enabled_channels: List[int]) -> bool:
        """
        执行连续测试

        Args:
            test_config: 测试配置
            enabled_channels: 启用的通道列表

        Returns:
            是否执行成功
        """
        try:
            logger.info("开始连续测试模式")

            # 获取连续测试配置
            continuous_mode = test_config.get('continuous_mode', False)
            count_limit_enabled = test_config.get('count_limit_enabled', False)
            max_count = test_config.get('max_count', 100)
            interval = test_config.get('interval', 2)

            logger.debug(f"连续测试配置: mode={continuous_mode}, limit={count_limit_enabled}, max={max_count}, interval={interval}s")

            # 修复检查连续测试模式是否正确启用
            if not continuous_mode:
                logger.error("❌ 连续测试模式未启用，无法执行连续测试")
                return False

            # 初始化连续测试计数和统计数据
            self.continuous_test_count = 0
            self.continuous_test_statistics = {
                'start_time': time.strftime("%Y-%m-%d %H:%M:%S"),
                'end_time': None,
                'cycle_times': [],
                'test_results': [],
                'total_cycles': 0
            }

            # 新增连续测试开始前预设增强电池码，确保阻抗明细数据使用正确的电池码
            logger.debug(f" 连续测试开始前预设增强电池码...")
            if self.test_result_manager and self.test_result_manager.impedance_data_manager:
                for channel_num in enabled_channels:
                    try:
                        # 获取原始电池码
                        original_battery_code = self.test_result_manager._get_current_battery_code(channel_num)
                        # 生成第1次测试的增强电池码
                        enhanced_battery_code = f"{original_battery_code}-T001"
                        # 预设到阻抗数据管理器
                        self.test_result_manager.impedance_data_manager.set_enhanced_battery_code(channel_num, enhanced_battery_code)
                        logger.debug(f" 预设通道{channel_num}增强电池码: {enhanced_battery_code}")
                    except Exception as e:
                        logger.error(f"❌ 预设通道{channel_num}增强电池码失败: {e}")

            # 修复发送连续测试启动事件
            logger.debug(f" 发送连续测试启动事件")
            if self.status_callback:
                try:
                    self.status_callback({
                        'action': 'continuous_test_started',
                        'count': self.continuous_test_count,
                        'max_count': max_count if count_limit_enabled else 0
                    })
                    logger.info(f"✅ 连续测试启动事件已发送")
                except Exception as e:
                    logger.error(f"❌ 发送连续测试启动事件失败: {e}")

            # 连续测试主循环
            logger.info(f"开始连续测试主循环，最大次数: {max_count if count_limit_enabled else '无限制'}")

            while not self.stop_event.is_set():
                # 增加测试计数
                self.continuous_test_count += 1
                logger.debug(f"开始第{self.continuous_test_count}轮连续测试")

                # 增强检查是否达到最大次数限制（在执行前检查）
                if count_limit_enabled and self.continuous_test_count > max_count:
                    logger.info(f"✅ 连续测试已达到最大次数限制: {max_count}，停止测试")
                    # 完成统计数据收集
                    self.continuous_test_statistics['end_time'] = time.strftime("%Y-%m-%d %H:%M:%S")
                    # 通知UI测试完成，包含统计数据
                    if self.status_callback:
                        self.status_callback({
                            'action': 'continuous_test_completed',
                            'count': self.continuous_test_count - 1,  # 减1因为这轮没有执行
                            'max_count': max_count,
                            'statistics': self.continuous_test_statistics
                        })
                    break

                # 修复在每轮测试开始前进行全面的状态清理
                logger.info(f"🧹 第{self.continuous_test_count}轮测试：开始状态清理")
                if self.status_callback:
                    self.status_callback({
                        'action': 'continuous_test_cycle_cleanup',
                        'count': self.continuous_test_count,
                        'max_count': max_count if count_limit_enabled else 0
                    })

                # 记录本轮测试开始时间
                cycle_start_time = time.time()

                logger.debug(f" 准备发送连续测试计数更新事件: count={self.continuous_test_count}, max_count={max_count if count_limit_enabled else 0}")

                if self.status_callback:
                    try:
                        self.status_callback({
                            'action': 'continuous_test_count_updated',
                            'count': self.continuous_test_count,
                            'max_count': max_count if count_limit_enabled else 0
                        })
                        logger.info(f"✅ 连续测试计数更新事件已发送: 第{self.continuous_test_count}轮")
                    except Exception as e:
                        logger.error(f"❌ 发送连续测试计数更新事件失败: {e}")
                else:
                    logger.warning("⚠️ status_callback 为空，无法发送计数更新事件")

                # 增强重新检查连续测试模式状态（防止运行时被禁用）
                current_continuous_mode = test_config.get('continuous_mode', False)
                if not current_continuous_mode:
                    logger.info("⚠️ 连续测试模式已被禁用，停止测试")
                    break

                # 新增每轮测试前更新增强电池码，确保序号正确
                if self.test_result_manager and self.test_result_manager.impedance_data_manager:
                    for channel_num in enabled_channels:
                        try:
                            # 获取原始电池码
                            original_battery_code = self.test_result_manager._get_current_battery_code(channel_num)
                            # 生成当前轮次的增强电池码
                            enhanced_battery_code = f"{original_battery_code}-T{self.continuous_test_count:03d}"
                            # 更新到阻抗数据管理器
                            self.test_result_manager.impedance_data_manager.set_enhanced_battery_code(channel_num, enhanced_battery_code)
                            logger.debug(f"🔧 第{self.continuous_test_count}轮：更新通道{channel_num}增强电池码: {enhanced_battery_code}")
                        except Exception as e:
                            logger.error(f"❌ 第{self.continuous_test_count}轮：更新通道{channel_num}增强电池码失败: {e}")

                # 执行一轮测试
                logger.info(f"🎯 执行第{self.continuous_test_count}轮测试...")
                success = self._execute_single_test(test_config, enabled_channels)

                # 记录本轮测试结束时间和统计数据
                cycle_end_time = time.time()
                cycle_duration = cycle_end_time - cycle_start_time
                self.continuous_test_statistics['cycle_times'].append(cycle_duration)
                self.continuous_test_statistics['total_cycles'] = self.continuous_test_count

                # 收集本轮测试结果（如果有的话）
                self._collect_cycle_test_results(enabled_channels)

                if success:
                    logger.info(f"✅ 第{self.continuous_test_count}轮测试完成，耗时: {cycle_duration:.2f}秒")
                else:
                    logger.warning(f"⚠️ 第{self.continuous_test_count}轮测试失败，继续下一轮")

                # 修复检查是否达到最大次数限制（在执行后检查）
                if count_limit_enabled and self.continuous_test_count >= max_count:
                    logger.info(f"✅ 连续测试已完成所有{max_count}轮测试")
                    # 完成统计数据收集
                    self.continuous_test_statistics['end_time'] = time.strftime("%Y-%m-%d %H:%M:%S")
                    # 通知UI测试完成，包含统计数据
                    if self.status_callback:
                        self.status_callback({
                            'action': 'continuous_test_completed',
                            'count': self.continuous_test_count,
                            'max_count': max_count,
                            'statistics': self.continuous_test_statistics
                        })
                    break

                # 等待间隔时间
                logger.info(f"⏱️ 等待{interval}秒后开始下一轮测试...")

                # 修复将浮点数转换为整数，分段等待，以便及时响应停止信号
                interval_int = int(interval)  # 转换为整数
                for i in range(interval_int):
                    if self.stop_event.is_set():
                        logger.info("🛑 收到停止信号，退出连续测试")
                        break
                    time.sleep(1)
                    if i % 5 == 0 and i > 0:  # 每5秒记录一次等待状态
                        logger.debug(f"⏳ 等待中... 剩余{interval_int - i}秒")

                # 修复如果有小数部分，额外等待
                decimal_part = interval - interval_int
                if decimal_part > 0 and not self.stop_event.is_set():
                    time.sleep(decimal_part)
                    logger.debug(f"⏳ 额外等待{decimal_part:.1f}秒完成")

            # 完成统计数据收集
            self.continuous_test_statistics['end_time'] = time.strftime("%Y-%m-%d %H:%M:%S")

            # 如果在停止标志下且有测试数据，通知UI显示报告
            if self.stop_event.is_set() and self.continuous_test_count > 0:
                logger.debug(f" 连续测试在停止标志下结束，有{self.continuous_test_count}轮测试数据，通知UI显示报告")
                if self.status_callback:
                    self.status_callback({
                        'action': 'continuous_test_stopped',
                        'count': self.continuous_test_count,
                        'max_count': max_count if count_limit_enabled else 0,
                        'statistics': self.continuous_test_statistics
                    })

            logger.info(f"连续测试模式结束，共完成{self.continuous_test_count}轮测试")
            return True

        except Exception as e:
            logger.error(f"连续测试执行失败: {e}")
            return False

    def _collect_cycle_test_results(self, enabled_channels: List[int]):
        """收集本轮测试结果数据"""
        try:
            logger.debug(f"开始收集第{self.continuous_test_count}轮测试结果数据")

            # 从测试结果管理器获取最新的测试结果
            if hasattr(self.test_result_manager, 'get_latest_test_results'):
                logger.debug("调用 get_latest_test_results 方法")
                latest_results = self.test_result_manager.get_latest_test_results(enabled_channels)

                if latest_results:
                    logger.debug(f"获取到 {len(latest_results)} 个通道的真实测试结果")
                    for result in latest_results:
                        result['cycle'] = self.continuous_test_count
                        self.continuous_test_statistics['test_results'].append(result)
                        logger.debug(f"通道{result['channel']} - Rs={result['rs_value']:.3f}mΩ, Rct={result['rct_value']:.3f}mΩ")
                else:
                    logger.debug("get_latest_test_results 返回空结果，使用模拟数据")
                    self._create_mock_test_results(enabled_channels)
            else:
                logger.debug("测试结果管理器没有 get_latest_test_results 方法，使用模拟数据")
                self._create_mock_test_results(enabled_channels)

        except Exception as e:
            logger.error(f"收集测试结果数据失败: {e}")
            import traceback
            logger.error(f"详细错误: {traceback.format_exc()}")
            # 出错时使用模拟数据
            self._create_mock_test_results(enabled_channels)

    def _create_mock_test_results(self, enabled_channels: List[int]):
        """创建模拟测试结果数据（包含EIS频点数据）"""
        try:
            logger.warning(f"🔍 DEBUG: 创建模拟测试结果数据（包含EIS频点数据）")

            # 定义模拟测试频率
            frequencies = [0.01, 0.1, 1.0, 10.0, 100.0, 1000.0, 7800.0]

            for channel in enabled_channels:
                # 创建频点数据
                frequency_data = []

                for freq in frequencies:
                    # 模拟EIS阻抗数据
                    import random
                    import math

                    # 简化的电池阻抗模型：Rs + Rct/(1 + j*ω*Rct*C)
                    rs = 5.0 + random.gauss(0, 0.2)  # Rs约5mΩ，有小幅波动
                    rct = 10.0 + random.gauss(0, 0.5)  # Rct约10mΩ，有中等波动
                    c = 1e-3  # 电容约1mF

                    omega = 2 * math.pi * freq
                    z_rct = rct / (1 + 1j * omega * rct * c)
                    z_total = rs + z_rct

                    # 添加通道间差异
                    channel_factor = 1.0 + (channel - 4.5) * 0.02  # 通道间2%差异
                    z_total *= channel_factor

                    # 添加测试轮次的微小漂移
                    cycle_drift = 1.0 + (self.continuous_test_count - 3) * 0.005  # 轮次间0.5%漂移
                    z_total *= cycle_drift

                    real_part = z_total.real
                    imag_part = z_total.imag
                    magnitude = abs(z_total)
                    # 修复相位角计算 - 不再对虚部取反，因为设备数据已经是正确符号
                    phase = math.degrees(math.atan2(imag_part, real_part))

                    frequency_data.append({
                        'frequency': freq,
                        'impedance_real': real_part,
                        'impedance_imag': imag_part,
                        'impedance_magnitude': magnitude,
                        'impedance_phase': phase
                    })

                # 计算Rs和Rct（从低频和高频阻抗估算）
                low_freq_z = frequency_data[0]['impedance_magnitude']  # 0.01Hz
                high_freq_z = frequency_data[-1]['impedance_magnitude']  # 7800Hz

                rs_value = high_freq_z  # 高频阻抗近似Rs
                rct_value = low_freq_z - high_freq_z  # 低频与高频差值近似Rct

                mock_result = {
                    'cycle': self.continuous_test_count,
                    'channel': channel,
                    'voltage': 3.7,  # 模拟电压
                    'rs_value': rs_value,  # 从阻抗数据计算的Rs值
                    'rct_value': rct_value,  # 从阻抗数据计算的Rct值
                    'rs_grade': 1,
                    'rct_grade': 2,
                    'is_pass': True,
                    'frequency_data': frequency_data,  # 新增EIS频点数据
                    'timestamp': f'2025-01-31 10:{self.continuous_test_count:02d}:{channel*5:02d}'
                }
                self.continuous_test_statistics['test_results'].append(mock_result)
                logger.debug(f"模拟数据 - 通道{channel}: Rs={mock_result['rs_value']:.1f}mΩ, Rct={mock_result['rct_value']:.1f}mΩ, 频点数={len(frequency_data)}")
        except Exception as e:
            logger.error(f"创建模拟测试结果失败: {e}")
            import traceback
            logger.error(f"详细错误信息: {traceback.format_exc()}")

    def _wait_for_measurement_completion(self, channel_indices: List[int], frequency: float) -> bool:
        """
        等待测量完成（智能超时优化版本）

        Args:
            channel_indices: 通道索引列表（0-7）
            frequency: 测试频率

        Returns:
            是否测量完成
        """
        try:
            # 🚀 性能优化：使用智能超时管理器
            if not hasattr(self, 'smart_timeout_manager'):
                from utils.smart_timeout_manager import SmartTimeoutManager
                self.smart_timeout_manager = SmartTimeoutManager()

            # 增强获取智能超时配置（考虑异常通道）
            # 计算最短超时时间（基于所有通道的状态）
            channel_timeouts = []
            for ch_idx in channel_indices:
                channel_num = ch_idx + 1
                if not self.exception_manager.is_channel_skipped(channel_num):
                    timeout, _ = self.smart_timeout_manager.get_smart_timeout(frequency, channel_num)
                    channel_timeouts.append(timeout)

            # 使用最短的超时时间，确保正常通道不被异常通道拖慢
            if channel_timeouts:
                measurement_timeout = min(channel_timeouts)
                logger.info(f"🎯 频点{frequency}Hz动态超时: {measurement_timeout:.1f}s (基于{len(channel_timeouts)}个正常通道)")
            else:
                # 如果所有通道都异常，使用默认超时
                measurement_timeout, _ = self.smart_timeout_manager.get_smart_timeout(frequency)
                logger.warning(f"🎯 频点{frequency}Hz所有通道异常，使用默认超时: {measurement_timeout:.1f}s")

            _, stability_check_interval = self.smart_timeout_manager.get_smart_timeout(frequency)
            measurement_start_time = time.time()
            last_progress_update = 0

            logger.info(f"🎯 频点{frequency}Hz智能超时: {measurement_timeout:.1f}s (稳定性检查间隔: {stability_check_interval:.1f}s)")

            while time.time() - measurement_start_time < measurement_timeout:
                if self.stop_event.is_set():
                    logger.info("检测到停止标志，停止本次测量")
                    # 记录实际测量时间
                    actual_time = time.time() - measurement_start_time
                    self.smart_timeout_manager.record_measurement_time(frequency, actual_time)
                    return False

                # 🚀 性能优化：使用更快的进度更新间隔
                elapsed_time = time.time() - measurement_start_time

                # 每1秒更新一次进度（优化后的频率）
                current_time = int(elapsed_time)
                if current_time > last_progress_update:
                    last_progress_update = current_time
                    if self.progress_callback:
                        enabled_channels = [ch_idx + 1 for ch_idx in channel_indices]
                        for channel_num in enabled_channels:
                            # 优化先检查通道是否已被标记为异常，避免不必要的查询
                            if self.exception_manager.is_channel_skipped(channel_num):
                                # 异常通道显示异常状态，不更新进度
                                exception_info = self.exception_manager.get_exception_info(channel_num)
                                if exception_info:
                                    self.progress_callback(channel_num, {
                                        'state': 'exception',
                                        'progress': 0,
                                        'message': f'通道异常: {exception_info.error_message}',
                                        'exception_type': exception_info.exception_type.value,
                                        'status_code': exception_info.status_code,
                                        'error_message': exception_info.error_message,
                                        'frequency': frequency
                                    })
                                continue  # 跳过异常通道的进度更新

                            # 对于正常通道，进行状态检查
                            channel_idx = channel_num - 1
                            status = self.comm_manager.get_measurement_status(channel_idx)

                            # 检查通道异常状态
                            is_channel_normal = self.exception_manager.check_channel_status(
                                channel_num, status, frequency
                            )

                            if not is_channel_normal:
                                # 新检测到的异常通道，跳过进度更新
                                continue

                            # 使用当前测试的频点索引
                            frequency_index = getattr(self, 'current_frequency_index', 1)

                            # 获取总频点数
                            try:
                                frequencies = self.test_config_manager.get_frequencies()
                                total_frequencies = len(frequencies) if frequencies else 20
                            except:
                                total_frequencies = 20

                            # 频点进度：测量中保持当前频点进度
                            freq_progress = 5 + ((frequency_index - 1) / total_frequencies * 90)
                            continuous_progress = freq_progress

                            # 修复使用锁定的测试前电压，而不是实时电压
                            voltage = self.pre_test_voltages.get(channel_num)
                            if voltage is None:
                                # 如果没有锁定电压，则读取当前电压作为备用
                                voltage = self.device_config_manager.read_channel_voltage(channel_num)
                                if voltage is None:
                                    voltage = 0.0
                                logger.debug(f"通道{channel_num}未找到锁定电压，使用实时电压: {voltage:.3f}V")
                            else:
                                logger.debug(f"通道{channel_num}使用锁定的测试前电压: {voltage:.3f}V")

                            self.progress_callback(channel_num, {
                                'state': 'measuring',
                                'progress': int(continuous_progress),
                                'message': f'测量中: {frequency}Hz ({elapsed_time:.1f}s)',
                                'frequency': frequency,
                                'frequency_index': frequency_index,
                                'total_frequencies': total_frequencies,
                                'voltage': voltage,
                                'voltage_locked': True,  # 标记使用锁定电压
                                'elapsed_time': elapsed_time,
                                'continuous_progress': True,
                                'time_based_progress': True  # 标记为基于时间的进度
                            })

                            # 调试日志
                            if int(elapsed_time) % 5 == 0:  # 每5秒打印一次
                                logger.debug(f"通道{channel_num}测量进度: {continuous_progress:.1f}% (已用时{elapsed_time:.1f}s)")

                # 🚀 性能优化：检查所有启用通道的测量状态
                                all_completed = True
                status_info = []
                normal_channels = []

                for ch_idx in channel_indices:
                    channel_num = ch_idx + 1
                    if self.exception_manager.is_channel_skipped(channel_num):
                        continue
                    status = self.comm_manager.get_measurement_status(ch_idx)
                    status_info.append("CH" + str(channel_num) + ":0x" + format(status, "04X"))
                    normal_channels.append(ch_idx)
                    if status != 0x0006:
                        all_completed = False

                if all_completed:
                    actual_time = time.time() - measurement_start_time
                    self.smart_timeout_manager.record_measurement_time(frequency, actual_time)
                    for ch_idx in normal_channels:
                        cn = ch_idx + 1
                        if self.progress_callback:
                            self.progress_callback(cn, {
                                'state': 'measurement_completed',
                                'progress': 100,
                                'message': '频点' + str(frequency) + 'Hz测量完成',
                                'frequency': frequency,
                                'measurement_time': actual_time
                            })
                    return True

                time.sleep(0.1)

            logger.warning(f"频点{frequency}Hz测量超时 (超时时间: {measurement_timeout:.1f}s)")
            # 记录超时时间
            actual_time = time.time() - measurement_start_time
            self.smart_timeout_manager.record_measurement_time(frequency, actual_time)
            return False

        except Exception as e:
            logger.error(f"等待测量完成失败: {e}")
            return False

    def _process_test_completion(self, channel_indices: List[int], test_config: Dict[str, Any]):
        """
        处理测试完成后的EIS分析和结果显示

        注意：此方法只在测试自然完成时被调用，因此应该保存数据
        用户手动停止测试时，测试循环会break，不会调用此方法

        Args:
            channel_indices: 通道索引列表（0-7）
            test_config: 测试配置
        """
        try:
            # 🔍 [DEBUG] 添加详细的调试日志
            logger.info(f"🔍 [DEBUG] _process_test_completion开始，通道索引: {channel_indices}")
            logger.info(f"🔍 [DEBUG] 通道号: {[ch + 1 for ch in channel_indices]}")
            logger.info(f"🔍 [DEBUG] 测试配置频点数: {len(test_config.get('frequencies', []))}")
            logger.info(f"🔍 [DEBUG] stop_event状态: {self.stop_event.is_set()}")
            # 修复检查是否为采样测试模式，采样测试模式下即使停止也要处理完成
            sampling_test = self.test_config_manager.config_manager.get('test.sampling_test', False)

            # Jack要求双重检查停止状态，确保不处理停止后的数据（但采样测试模式和单频点测试例外）
            # 🔧 [单频点修复] 检查是否为单频点测试
            is_single_freq_test = len(test_config.get('frequencies', [])) == 1

            if not sampling_test and not is_single_freq_test and self.stop_event.is_set():
                logger.warning("🛑 检测到测试已被停止，跳过测试完成处理，避免计算脏数据")
                return
            elif sampling_test and self.stop_event.is_set():
                logger.info("🎯 采样测试模式：即使测试停止也继续处理完成，确保数据能正确显示")
            elif is_single_freq_test and self.stop_event.is_set():
                logger.info("🔧 [单频点测试] 即使测试停止也继续处理完成，确保单频点数据能正确保存")

            logger.info("开始处理测试完成后的EIS分析...")

            # 增强只处理正常通道的测试结果
            normal_channels = []
            exception_channels = []

            for ch_idx in channel_indices:
                channel_num = ch_idx + 1
                if self.exception_manager.is_channel_skipped(channel_num):
                    exception_channels.append(channel_num)
                else:
                    normal_channels.append(ch_idx)

            logger.info(f"测试结果处理: 正常通道{len(normal_channels)}个, 异常通道{len(exception_channels)}个")
            if exception_channels:
                logger.info(f"异常通道列表: {exception_channels}")

            # 优化只处理正常通道的测试结果
            for ch_idx in normal_channels:
                channel_num = ch_idx + 1

                # 计算Rs和Rct值，处理阻抗跳变异常
                try:
                    rs_value, rct_value = self.test_result_manager.calculate_rs_rct_for_channel(channel_num)
                except ValueError as e:
                    # 检查是否是阻抗跳变异常
                    if "阻抗数据存在严重跳变" in str(e):
                        logger.error(f"通道{channel_num}阻抗数据跳变检测失败: {e}")

                        # 标记为数据错误，跳过此通道的正常处理
                        error_result = {
                            'channel': channel_num,
                            'batch_id': self.test_result_manager.current_batch_id,
                            'test_status': 'DATA_ERROR',
                            'rs': 0.0,
                            'rct': 0.0,
                            'w_impedance': 0.0,
                            'error_message': str(e),
                            'error_type': 'impedance_jump',
                            'failure_reason': f'阻抗数据跳变: {str(e)}',
                            'voltage': self.pre_test_voltages.get(channel_num, 0.0),
                            'test_start_time': self.test_result_manager._get_current_timestamp(),
                            'test_end_time': self.test_result_manager._get_current_timestamp(),
                            'grade': 'ERROR'
                        }

                        # 保存错误结果
                        self.test_result_manager.save_test_result(error_result)

                        # 通知UI更新
                        if self.progress_callback:
                            self.progress_callback(channel_num, {
                                'state': 'failed',
                                'error': '阻抗数据跳变',
                                'details': str(e),
                                'rs_value': 0.0,
                                'rct_value': 0.0,
                                'voltage': self.pre_test_voltages.get(channel_num, 0.0)
                            })

                        logger.error(f"❌ 通道{channel_num}因阻抗跳变标记为数据错误，跳过正常处理")
                        continue  # 跳过此通道的后续处理
                    else:
                        # 其他ValueError异常，重新抛出
                        raise e

                # 修复获取Rsei值
                rsei_value = self.test_result_manager.get_channel_rsei_value(channel_num)

                # 修复获取增强版EIS分析结果
                enhanced_analysis = self.test_result_manager.get_channel_enhanced_analysis(channel_num)

                # 修改使用测试前保存的电压值（静置状态下的准确电压）
                voltage = self.pre_test_voltages.get(channel_num)
                if voltage is None:
                    # 如果没有保存的电压值，则读取当前电压作为备用
                    voltage = self.device_config_manager.read_channel_voltage(channel_num)
                    logger.warning(f"⚠️ 通道{channel_num}未找到测试前电压，使用当前电压: {voltage:.3f}V")
                else:
                    logger.info(f"✅ 通道{channel_num}使用测试前保存的电压: {voltage:.3f}V")

                # 🎯 取样测试模式：跳过判断逻辑
                sampling_test = self.test_config_manager.config_manager.get('test.sampling_test', False)
                if sampling_test:
                    # 取样测试模式：不进行判断，设置默认值
                    logger.info(f"🎯 通道{channel_num}取样测试模式：跳过测试执行器判断逻辑")
                    rs_grade = 0  # 取样测试不计算档位
                    rct_grade = 0
                    is_pass = None  # 取样测试不判断合格性
                    fail_items = []
                else:
                    # 正常测试模式：计算档位和判断合格性
                    logger.info(f"🔍 [档位计算前] 通道{channel_num}: Rs={rs_value:.3f}mΩ, Rct={rct_value:.3f}mΩ")
                    rs_grade, rct_grade = self.test_result_manager.calculate_grades(rs_value, rct_value)
                    logger.info(f"🔍 [档位计算后] 通道{channel_num}: Rs档位={rs_grade}, Rct档位={rct_grade}")
                    is_pass, fail_items = self.test_result_manager.judge_test_result(voltage, rs_value, rct_value)

                # 🔧 修复：准备完整的结果数据，包含outlier_result字段
                result_data = {
                    'voltage': voltage,
                    'voltage_source': 'pre_test',  # 标记电压来源为测试前保存的电压
                    'rs_value': rs_value,
                    'rct_value': rct_value,
                    'rsei_value': rsei_value,  # 添加Rsei值
                    'rs_grade': rs_grade,
                    'rct_grade': rct_grade,
                    'is_pass': is_pass,
                    'fail_items': fail_items,
                    'state': 'completed',
                    'outlier_result': None  # 🔧 修复：添加outlier_result字段，避免失败原因管理器异常
                }

                # 修复添加阻抗明细数据 (frequency_data)
                try:
                    if self.impedance_data_manager:
                        # 获取该通道的阻抗数据
                        channel_impedance_data = self.impedance_data_manager.get_channel_impedance_data(channel_num)
                        if channel_impedance_data:
                            # 转换为EIS分析器期望的格式
                            frequency_data = []
                            for freq, impedance_info in channel_impedance_data.items():
                                if isinstance(impedance_info, dict):
                                    real_imp = impedance_info.get('real', 0.0) / 1000.0  # μΩ转换为mΩ
                                    imag_imp = impedance_info.get('imag', 0.0) / 1000.0  # μΩ转换为mΩ
                                    magnitude = (real_imp**2 + imag_imp**2)**0.5
                                    phase = math.degrees(math.atan2(imag_imp, real_imp)) if real_imp != 0 else 0.0

                                    frequency_data.append({
                                        'frequency': freq,
                                        'impedance_real': real_imp,
                                        'impedance_imag': imag_imp,
                                        'impedance_magnitude': magnitude,
                                        'impedance_phase': phase
                                    })

                            result_data['frequency_data'] = frequency_data
                            logger.info(f"通道{channel_num}添加{len(frequency_data)}个频点的阻抗明细数据到result_data")
                        else:
                            logger.warning(f"通道{channel_num}没有找到阻抗明细数据")
                            result_data['frequency_data'] = []
                    else:
                        logger.warning(f"阻抗数据管理器不可用，无法获取通道{channel_num}的阻抗明细数据")
                        result_data['frequency_data'] = []
                except Exception as e:
                    logger.error(f"获取通道{channel_num}阻抗明细数据失败: {e}")
                    result_data['frequency_data'] = []

                # 修复添加增强版EIS分析结果
                if enhanced_analysis:
                    result_data['enhanced_analysis'] = enhanced_analysis
                    logger.debug(f"通道{channel_num}添加增强版EIS分析结果到result_data")

                # 修复连续测试模式下的数据库保存增强
                continuous_mode = test_config.get('continuous_mode', False)
                if continuous_mode:
                    cycle_count = getattr(self, 'continuous_test_count', 0)
                    logger.info(f"🔄 连续测试第{cycle_count}轮：准备保存通道{channel_num}测试结果到数据库")
                    logger.info(f"📊 连续测试数据：Rs={rs_value:.3f}mΩ, Rct={rct_value:.3f}mΩ, 电压={voltage:.3f}V")

                    # 确保测试结果管理器知道当前的连续测试计数
                    if hasattr(self.test_result_manager, 'continuous_test_count'):
                        self.test_result_manager.continuous_test_count = cycle_count
                else:
                    logger.info(f"准备保存通道{channel_num}测试结果到数据库")

                # 保存测试结果到数据库（测试自然完成，必须保存数据）
                try:
                    # 🔍 [DEBUG] 添加数据保存调试日志
                    logger.info(f"🔍 [DEBUG] 准备保存通道{channel_num}数据到数据库")
                    logger.info(f"🔍 [DEBUG] Rs={result_data.get('rs_value', 0):.3f}mΩ, Rct={result_data.get('rct_value', 0):.3f}mΩ")

                    # 🔍 [SINGLE_FREQ_DEBUG] 单频点数据保存调试
                    if result_data.get('analysis_method') == 'single_frequency_mode':
                        logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 准备保存单频点测试数据:")
                        logger.info(f"🔍 [SINGLE_FREQ_DEBUG]   通道: {channel_num}")
                        logger.info(f"🔍 [SINGLE_FREQ_DEBUG]   Rs: {result_data.get('rs_value', 0):.3f}mΩ")
                        logger.info(f"🔍 [SINGLE_FREQ_DEBUG]   Rct: {result_data.get('rct_value', 0):.3f}mΩ")
                        logger.info(f"🔍 [SINGLE_FREQ_DEBUG]   分析方法: {result_data.get('analysis_method')}")

                    self.test_result_manager.save_test_result(channel_num, result_data, test_config)

                    logger.info(f"🔍 [DEBUG] 通道{channel_num}数据保存调用完成")

                    # 🔍 [SINGLE_FREQ_DEBUG] 单频点保存完成调试
                    if result_data.get('analysis_method') == 'single_frequency_mode':
                        logger.info(f"🔍 [SINGLE_FREQ_DEBUG] 单频点数据保存调用完成")

                    if continuous_mode:
                        cycle_count = getattr(self, 'continuous_test_count', 0)
                        logger.info(f"✅ 连续测试第{cycle_count}轮：通道{channel_num}测试结果已保存到数据库")
                    else:
                        logger.info(f"✅ 通道{channel_num}测试结果已保存到数据库")

                except Exception as save_e:
                    if continuous_mode:
                        cycle_count = getattr(self, 'continuous_test_count', 0)
                        logger.error(f"❌ 连续测试第{cycle_count}轮：通道{channel_num}测试结果保存失败: {save_e}")
                    else:
                        logger.error(f"❌ 通道{channel_num}测试结果保存失败: {save_e}")
                    raise

                # 🎯 取样测试模式：跳过UI回调，但数据已保存
                if not sampling_test:
                    # 发送结果回调（包含fail_items和Rsei值）
                    if self.progress_callback:
                        self.progress_callback(channel_num, {
                            'state': 'completed',
                            'progress': 100,
                            'message': '测试完成',
                            'voltage': voltage,
                            'rs_value': rs_value,
                            'rct_value': rct_value,
                            'rsei_value': rsei_value,  # 添加Rsei值到回调
                            'rs_grade': rs_grade,
                            'rct_grade': rct_grade,
                            'is_pass': is_pass,
                            'fail_items': result_data.get('fail_items', []),
                            'fail_reason': result_data.get('fail_reason', ''),
                            'result_data': result_data
                        })
                        logger.info(f"✅ 通道{channel_num}测试完成回调已发送")
                else:
                    # 关键修复取样测试模式下发送特殊回调，用于设置测试结束时间和发射信号
                    if self.progress_callback:
                        self.progress_callback(channel_num, {
                            'state': 'sampling_completed',  # 特殊状态标识取样测试完成
                            'progress': 100,
                            'message': '取样测试完成',
                            'voltage': voltage,
                            'rs_value': rs_value,
                            'rct_value': rct_value,
                            'rsei_value': rsei_value,
                            'sampling_mode': True  # 标识为取样模式
                        })
                        logger.info(f"🎯 通道{channel_num}取样测试模式：发送特殊回调用于设置结束时间")
                    else:
                        logger.info(f"🎯 通道{channel_num}取样测试模式：跳过UI回调，数据已保存到数据库")

                # 🚫 离群检测功能已暂时禁用
                # 发送最终离群率结果（批量测试完成后）
                # try:
                # if hasattr(self.impedance_data_manager, 'channel_frequency_deviations'):
                # frequency_deviations = self.impedance_data_manager.channel_frequency_deviations.get(channel_num, {})
                #
                # if frequency_deviations:
                # # 计算最终离群率结果
                # from backend.outlier_detection_manager import OutlierDetectionManager
                # outlier_manager = OutlierDetectionManager()
                # config = outlier_manager.get_detection_config()
                #
                # if config.get('is_enabled', False):
                # threshold = config.get('deviation_threshold', 10.0)
                # max_deviation = max(frequency_deviations.values())
                #
                # if max_deviation <= threshold:
                # final_outlier_result = "PASS"
                # else:
                # final_outlier_result = f"{max_deviation:.1f}%"
                #
                # # 发送最终离群率结果到UI
                # callback_data = {
                # 'state': 'final_outlier_rate_update',
                # 'outlier_result': final_outlier_result,
                # 'baseline_filename': f"基准_{config['active_baseline_id']}.json",
                # 'frequency_deviations': frequency_deviations.copy(),
                # 'max_deviation': max_deviation,
                # 'threshold': threshold
                # }
                # logger.debug(f"通道{channel_num} 发送最终离群率UI更新")
                # if self.progress_callback:
                # self.progress_callback(channel_num, callback_data)
                # logger.debug(f"通道{channel_num} 最终离群率UI更新发送成功")
                #
                # except Exception as e:
                # logger.error(f"发送通道{channel_num}最终离群率结果失败: {e}")

                logger.info(f"通道{channel_num}测试结果: Rs={rs_value:.3f}mΩ, Rct={rct_value:.3f}mΩ, "
                           f"档位={rs_grade}-{rct_grade}, 结果={'合格' if is_pass else '不合格'}")

            # 新增处理异常通道的结果显示
            for channel_num in exception_channels:
                try:
                    exception_info = self.exception_manager.get_exception_info(channel_num)
                    if exception_info:
                        # 获取异常原因的中文描述
                        if exception_info.exception_type.value == 'contact_poor':
                            fail_reason = '不合格-接触不良'
                        elif exception_info.exception_type.value == 'battery_error':
                            fail_reason = '不合格-电池异常'
                        else:
                            fail_reason = f'不合格-{exception_info.error_message}'

                        # 🔧 修复：准备异常通道的结果数据，包含outlier_result字段
                        exception_result_data = {
                            'voltage': exception_info.voltage_when_detected or 0.0,
                            'rs_value': 0.0,
                            'rct_value': 0.0,
                            'rs_grade': '--',
                            'rct_grade': '--',
                            'is_pass': False,
                            'fail_items': ['异常'],
                            'fail_reason': fail_reason,
                            'state': 'exception',
                            'exception_type': exception_info.exception_type.value,
                            'error_message': exception_info.error_message,
                            'outlier_result': None  # 🔧 修复：添加outlier_result字段，避免失败原因管理器异常
                        }

                        # 保存异常通道结果到数据库
                        self.test_result_manager.save_test_result(channel_num, exception_result_data, test_config)

                        # 发送异常通道结果回调
                        if self.progress_callback:
                            self.progress_callback(channel_num, {
                                'state': 'completed',
                                'progress': 100,
                                'message': fail_reason,
                                'voltage': exception_result_data['voltage'],
                                'rs_value': 0.0,
                                'rct_value': 0.0,
                                'rs_grade': '--',
                                'rct_grade': '--',
                                'is_pass': False,
                                'fail_items': exception_result_data['fail_items'],
                                'fail_reason': fail_reason,
                                'result_data': exception_result_data
                            })

                        logger.info(f"通道{channel_num}异常结果: {fail_reason}")

                except Exception as e:
                    logger.error(f"处理通道{channel_num}异常结果失败: {e}")

            # 🚀 Jack的智能频点优化学习
            try:
                if test_config.get('optimization_used', False):
                    logger.info("🎓 使用了优化频点，跳过学习（已有模板）")
                # 🚫 频点优化学习代码已删除
            except Exception as e:
                logger.error(f"频点优化学习失败: {e}")

        except Exception as e:
            logger.error(f"处理测试完成失败: {e}")

    def get_execution_status(self) -> Dict[str, Any]:
        """
        获取执行状态

        Returns:
            执行状态字典
        """
        return {
            'is_stopped': self.stop_event.is_set(),
            'startup_stats': self.startup_strategy_manager.get_startup_stats(),
            'impedance_data_summary': self.impedance_data_manager.get_impedance_data_summary(),
            'batch_info': self.test_result_manager.get_batch_info()
        }

    def stop_execution(self):
        """停止测试执行"""
        try:
            # 修复：添加调用栈深度检查，防止递归调用
            import inspect
            current_frame = inspect.currentframe()
            call_count = 0
            frame = current_frame
            while frame:
                if frame.f_code.co_name == 'stop_execution':
                    call_count += 1
                frame = frame.f_back

            if call_count > 1:
                logger.warning(f"🛑 检测到递归调用 stop_execution (深度: {call_count})，跳过执行")
                return

            logger.info("🛑 测试执行器开始停止测试...")

            # 修复1设置停止事件
            self.stop_event.set()
            logger.info("🛑 测试执行停止信号已发送")

            # 修复2立即停止设备测试
            if hasattr(self, 'comm_manager') and self.comm_manager:
                logger.info("🛑 正在停止设备测试...")
                # 停止所有通道的测试
                all_channels = list(range(8))  # 0-7对应通道1-8
                stop_success = self.comm_manager.stop_impedance_measurement(all_channels)
                if stop_success:
                    logger.info("✅ 设备测试已成功停止")
                else:
                    logger.warning("⚠️ 设备测试停止失败，但软件停止信号已发送")
            else:
                logger.warning("⚠️ 通信管理器不可用，仅发送软件停止信号")

            # 修复3停止所有测试管理器（如果存在）
            self._stop_active_test_managers()

            # Jack要求停止测试时清理数据，不计算Rs/Rct，不打印，不存数据库
            self._cleanup_stopped_test_data()

            # 修复停止测试后发送状态回调，通知UI测试已停止
            if self.status_callback:
                try:
                    logger.info("🛑 测试停止完成，发送状态回调通知")
                    self.status_callback({'is_testing': False, 'action': 'test_stopped', 'reason': '停止标志触发'})
                    logger.debug("✅ 测试停止状态回调已发送")
                except Exception as e:
                    logger.error(f"❌ 发送测试停止状态回调失败: {e}")
            else:
                logger.warning("⚠️ status_callback 为空，无法发送测试停止通知")

            logger.info("✅ 测试执行器停止完成")

        except Exception as e:
            logger.error(f"停止测试执行时发生错误: {e}")
            import traceback
            logger.error(f"详细错误信息: {traceback.format_exc()}")
            # 确保停止事件被设置
            self.stop_event.set()

    def _stop_active_test_managers(self):
        """停止活跃的测试管理器"""
        try:
            logger.debug("🛑 停止活跃的测试管理器...")

            # 停止并行错频测试管理器（如果存在）
            if hasattr(self, '_current_staggered_manager') and self._current_staggered_manager:
                try:
                    self._current_staggered_manager.stop_test()
                    logger.info("✅ 并行错频测试管理器已停止")
                except Exception as e:
                    logger.error(f"停止并行错频测试管理器失败: {e}")

            # 停止同时测试管理器（如果存在）
            if hasattr(self, '_current_simultaneous_manager') and self._current_simultaneous_manager:
                try:
                    self._current_simultaneous_manager.stop_simultaneous_test()
                    logger.info("✅ 同时测试管理器已停止")
                except Exception as e:
                    logger.error(f"停止同时测试管理器失败: {e}")

        except Exception as e:
            logger.error(f"停止测试管理器失败: {e}")

    def reset_execution(self):
        """重置执行状态"""
        self.stop_event.clear()
        self.startup_strategy_manager.reset_startup_stats()
        logger.info("测试执行状态已重置")

    def _cleanup_stopped_test_data(self):
        """
        清理停止测试时的数据

        当检测到停止标志时，不计算Rs/Rct，不打印结果，不存数据库
        因为这些是不完整的脏数据，应该被丢弃
        """
        try:
            logger.info("🛑 开始清理停止测试时的数据...")

            # 获取当前启用的通道
            enabled_channels = self.test_config_manager.get_enabled_channels()
            if not enabled_channels:
                logger.info("🛑 没有启用的通道，跳过数据清理")
                return

            # 清理阻抗数据
            if hasattr(self, 'impedance_data_manager') and self.impedance_data_manager:
                try:
                    # 获取当前批次ID
                    batch_id = self.test_result_manager.current_batch_id if hasattr(self, 'test_result_manager') else None

                    if batch_id:
                        logger.info(f"🛑 清理批次{batch_id}的阻抗数据...")
                        # 清理当前批次的阻抗数据
                        self.impedance_data_manager.clear_batch_data(batch_id)
                        logger.info("✅ 阻抗数据已清理")
                    else:
                        logger.warning("⚠️ 未找到批次ID，跳过阻抗数据清理")

                except Exception as e:
                    logger.error(f"清理阻抗数据失败: {e}")

            # 清理测试结果数据
            if hasattr(self, 'test_result_manager') and self.test_result_manager:
                try:
                    logger.info("🛑 清理测试结果数据...")
                    # 清理当前测试的结果数据，但不删除数据库记录
                    for channel_num in enabled_channels:
                        # 重置通道的计算结果
                        if hasattr(self.test_result_manager, 'reset_channel_results'):
                            self.test_result_manager.reset_channel_results(channel_num)

                        # 发送停止状态到UI，但不包含计算结果
                        if self.progress_callback:
                            try:
                                stop_data = {
                                    'state': 'stopped',
                                    'stop_reason': '停止标志触发',
                                    'voltage': 0.0,
                                    'rs_value': 0.0,
                                    'rct_value': 0.0,
                                    'progress': 0,
                                    'no_calculation': True  # 标记为不计算结果
                                }
                                self.progress_callback(channel_num, stop_data)
                                logger.debug(f"🛑 通道{channel_num}停止状态已发送（无计算结果）")
                            except Exception as e:
                                logger.error(f"🛑 通道{channel_num}发送停止状态失败: {e}")

                    logger.info("✅ 测试结果数据已清理")

                except Exception as e:
                    logger.error(f"清理测试结果数据失败: {e}")

            # 清理测试前电压数据
            if hasattr(self, 'pre_test_voltages'):
                try:
                    self.pre_test_voltages.clear()
                    logger.info("✅ 测试前电压数据已清理")
                except Exception as e:
                    logger.error(f"清理测试前电压数据失败: {e}")

            # 重置测试状态
            try:
                # 重置异常管理器状态
                if hasattr(self, 'exception_manager') and self.exception_manager:
                    self.exception_manager.reset_all_channels()

                logger.info("✅ 测试状态已重置")
            except Exception as e:
                logger.error(f"重置测试状态失败: {e}")

            logger.info("🛑 停止测试数据清理完成 - 不计算Rs/Rct，不打印，不存数据库")

        except Exception as e:
            logger.error(f"🛑 清理停止测试数据失败: {e}")
            import traceback
            logger.error(f"详细错误: {traceback.format_exc()}")

    # 🚫 智能频点优化方法已删除

    # 🚫 电芯型号标识获取方法已删除

    # 🚫 频点优化学习方法已删除
