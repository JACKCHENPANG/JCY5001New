...
class TestFlowController:
    ...
    def __init__(self, config_manager, comm_manager, progress_callback=None, status_callback=None):
        ...
        logger.debug("重构后的测试流程控制器初始化完成")
    
    def _initialize_managers(self):
        """初始化各个管理器"""
        try:
            # 修复保存现有的数据上传管理器引用
            existing_data_upload_manager = getattr(self, '_data_upload_manager', None)
            ...
            # 4. 测试结果管理器（传递现有的数据上传管理器）
            self.test_result_manager = TestResultManager(
                self.config_manager,
                self.impedance_data_manager,
                existing_data_upload_manager  # 传递现有的数据上传管理器
            )

            # 🔥 设置主窗口引用（用于自动打印）
            if self.main_window:
                self.test_result_manager.main_window = self.main_window

            # 5. 取样测试管理器
            self.sampling_test_manager = SamplingTestManager(self.config_manager)
            # 确保初始化时计数为0
            self.sampling_test_manager.reset_all_counts()

            # 数据上传管理器（保持现有引用）
            self._data_upload_manager = existing_data_upload_manager
    ...
    def set_data_upload_manager(self, upload_manager):
        ...
        """
        try:
            logger.debug(f" [流程控制器] 开始设置数据上传管理器: {upload_manager is not None}")
            self._data_upload_manager = upload_manager

            # 检查测试结果管理器状态
            result_manager_exists = self.test_result_manager is not None
            logger.debug(f" [流程控制器] 测试结果管理器状态: exists={result_manager_exists}")

            # 设置到测试结果管理器
            if self.test_result_manager:
                logger.debug(f" [流程控制器] 正在设置数据上传管理器到测试结果管理器...")
                self.test_result_manager.set_data_upload_manager(upload_manager)
                logger.info("✅ 数据上传管理器已设置到测试流程控制器和测试结果管理器")

                # 验证设置是否成功
                verify_manager = getattr(self.test_result_manager, 'data_upload_manager', None)
                logger.debug(f" [流程控制器] 验证设置结果: {verify_manager is not None}")
            else:
                logger.warning("⚠️ 测试结果管理器未初始化，无法设置数据上传管理器")

            # 修复如果后续重新初始化管理器，确保数据上传管理器不会丢失
            logger.debug(f" [流程控制器] 数据上传管理器已保存到_data_upload_manager: {self._data_upload_manager is not None}")
    ...
    def _sync_compatibility_attributes(self):
        ...
        try:
            ...
            if self.battery_codes:
                self.impedance_data_manager.set_battery_codes(self.battery_codes)
                logger.info(f"✅ 已设置电池码到阻抗数据管理器: {len(self.battery_codes)}个电池码 - {self.battery_codes}")
            else:
                # 修复降低日志级别，避免在初始化阶段产生误导性警告
                logger.debug(f"电池码列表为空，阻抗数据管理器将在测试开始时设置电池码")
    ...
    def start_batch_test(self, batch_info: Optional[Dict] = None, battery_codes: Optional[List[str]] = None) -> bool:
        ...
        """
        try:
            # 🚀 [快速修复] 强制重置所有状态，确保第二次启动成功
            if self.is_testing or (hasattr(self, 'current_state') and self.current_state != 'idle'):
                logger.warning(f"🚀 [快速修复] 检测到状态异常，强制重置: is_testing={self.is_testing}, current_state={getattr(self, 'current_state', 'N/A')}")
                ...
                if hasattr(self, 'test_thread') and self.test_thread:
                    if self.test_thread.is_alive():
                        logger.warning("🚀 [快速修复] 发现活跃的测试线程，等待结束")
                        try:
                ...
                logger.info("🚀 [快速修复] 状态强制重置完成")

            if self.is_testing:
                logger.warning("测试已在进行中 ❌")
                logger.debug(f"🔍 当前状态详情: is_testing={self.is_testing}, current_state={self.current_state}")
                return False

            # 新增检查测试线程状态
            if self.test_thread and self.test_thread.is_alive():
                logger.warning("测试线程仍在运行，强制等待结束 ⚠️")
                try:
                    self.test_thread.join(timeout=2.0)  # 等待最多2秒
                    if self.test_thread.is_alive():
                        logger.error("测试线程无法正常结束，强制重置状态 ❌")
                        self.is_testing = False
                        self.current_state = TestState.IDLE
                        self.stop_event.clear()
                except Exception as e:
                    logger.error(f"等待测试线程结束失败: {e}")
                    # 强制重置状态
                    self.is_testing = False
                    self.current_state = TestState.IDLE
                    self.stop_event.clear()
    ...
    def start_sampling_test(self, sample_count: int) -> bool:
        ...
        try:
            if self.is_testing:
                logger.warning("测试已在进行中，无法启动取样测试")
                return False

            # 启动取样测试管理器
            if not self.sampling_test_manager.start_sampling_test(sample_count):
                logger.error("❌ 启动取样测试管理器失败")
                return False

            logger.info(f"✅ 取样测试已启动，目标样本数: {sample_count}")
            return True
    ...
    def stop_sampling_test(self):
        """停止取样测试"""
        try:
            self.sampling_test_manager.stop_sampling_test()
            logger.info("✅ 取样测试已停止")
        except Exception as e:
    ...
    def is_sampling_mode(self) -> bool:
        """检查是否为取样测试模式"""
        return self.sampling_test_manager.is_sampling_mode

    def get_sampling_progress(self) -> tuple:
        """获取取样测试进度"""
        return self.sampling_test_manager.get_progress_info()

    @handle_exceptions(show_dialog=True, auto_retry=False, critical=False, fallback_value=None)
    def stop_test(self):
        """停止测试（增强版）"""
        # 修复：使用线程锁防止重复执行
        with self._stop_lock:
            if self._stop_in_progress:
                logger.warning("🛑 停止操作已在进行中，跳过重复调用")
                return
        ...
        try:
            ...
            if self.test_thread and self.test_thread.is_alive():
                ...
                try:
                    self.test_thread.join(timeout=5.0)  # 等待最多5秒
                    if self.test_thread.is_alive():
                        logger.warning("⚠️ 测试线程未能在5秒内结束，强制清理状态")
                    else:
    ...
    def _test_worker(self, test_config: Dict):
        ...
        try:
            ...
            # 修复在测试开始前确保频点序号映射和电池码已设置

            # 设置测试频点到阻抗数据管理器
            frequencies = test_config.get('frequencies', [])
            if frequencies and hasattr(self.impedance_data_manager, 'set_test_frequencies'):
                self.impedance_data_manager.set_test_frequencies(frequencies)
                logger.info(f"✅ 测试工作线程中设置频点序号映射: {len(frequencies)}个频点")

            # 确保电池码已设置
            if self.battery_codes and hasattr(self.impedance_data_manager, 'set_battery_codes'):
                self.impedance_data_manager.set_battery_codes(self.battery_codes)
                logger.info(f"✅ 测试工作线程中设置电池码: {len(self.battery_codes)}个电池码")
            else:
                logger.warning(f"测试工作线程中电池码为空: {self.battery_codes}")
                # 尝试从测试结果管理器获取电池码
                batch_info = self.test_result_manager.get_batch_info()
                backup_battery_codes = batch_info.get('battery_codes', [])
                if backup_battery_codes:
                    self.battery_codes = backup_battery_codes
                    self.impedance_data_manager.set_battery_codes(self.battery_codes)
                    logger.info(f"从测试结果管理器获取电池码: {len(self.battery_codes)}个电池码")
            ...
            if success:
                ...
                if self.stop_event.is_set():
                    logger.info("🎉 测试工作线程正常完成（检测到停止标志）")
                else:
                    logger.info("🎉 测试工作线程正常完成")
            else:
                if self.stop_event.is_set():
                    self.current_state = TestState.STOPPED
                    logger.info("🛑 测试工作线程已停止")
                else:
                    self.current_state = TestState.FAILED
                    logger.warning("⚠️ 测试工作线程异常结束")
