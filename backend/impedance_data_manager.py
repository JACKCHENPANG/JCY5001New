...
class ImpedanceDataManager:
        ...
        try:
            ...
            try:
                with self.db_manager.get_connection() as conn:
                    ...
                    if batch_exists == 0:
                        logger.warning(f"⚠️ 批次ID {batch_id} 不存在，尝试创建...")
                        # 自动创建缺失的批次
                        cursor.execute('''
                            INSERT OR IGNORE INTO batches (id, batch_number, created_at, total_count, pass_count, fail_count)
                            VALUES (?, ?, datetime('now'), 0, 0, 0)
                        ''', (batch_id, f'BATCH-AUTO-{batch_id}'))
                        conn.commit()
                        logger.info(f"✅ 批次 {batch_id} 自动创建成功")
                    else:
                        logger.debug(f"🔍 [DEBUG] 批次ID {batch_id} 存在，可以保存阻抗数据")
            except Exception as batch_e:
                logger.error(f"❌ 批次检查失败: {batch_e}")
                # 继续执行，让数据库自己处理外键约束
    ...
    def validate_frequency_data_completeness(self, expected_frequencies: List[float], channel_num: int) -> Dict[str, Any]:
        """
        验证指定通道的频点数据完整性
        ...
        try:
            ...
            # 获取该通道的阻抗数据
            channel_data = self.get_channel_impedance_data(channel_num)

            if not channel_data:
                result.update({
                    'is_complete': False,
                    'severity': 'critical',
                    'message': f'通道{channel_num}无任何阻抗数据'
                })
                return result

            # 检查实际采集的频点
            actual_frequencies = list(channel_data.keys())
            result['actual_count'] = len(actual_frequencies)

            # 找出缺失的频点
            missing_frequencies = [freq for freq in expected_frequencies if freq not in actual_frequencies]
            result['missing_frequencies'] = missing_frequencies
            result['missing_count'] = len(missing_frequencies)

            if result['missing_count'] == 0:
                result['message'] = f'通道{channel_num}频点数据完整'
                return result

            # 计算缺失比例和严重程度
            result['missing_ratio'] = result['missing_count'] / result['expected_count']
            result['is_complete'] = False

            if result['missing_ratio'] < 0.1:  # 缺失少于10%
                result['severity'] = 'minor'
                result['message'] = f'通道{channel_num}轻微缺失{result["missing_count"]}个频点'
            elif result['missing_ratio'] < 0.3:  # 缺失10%-30%
                result['severity'] = 'major'
                result['message'] = f'通道{channel_num}严重缺失{result["missing_count"]}个频点'
            else:  # 缺失超过30%
                result['severity'] = 'critical'
                result['message'] = f'通道{channel_num}数据严重不完整，缺失{result["missing_count"]}个频点'

            return result

        except Exception as e:
            logger.error(f"验证通道{channel_num}频点数据完整性失败: {e}")
            return {
                'is_complete': False,
                'expected_count': len(expected_frequencies),
                'actual_count': 0,
                'missing_count': len(expected_frequencies),
                'missing_frequencies': expected_frequencies,
                'missing_ratio': 1.0,
                'severity': 'critical',
                'message': f'通道{channel_num}验证失败: {str(e)}'
            }
